...
Remove the `superflow` entry from `mcp.json` to disconnect Cursor. To revoke the grant itself, disconnect Superflow from your [Superflow dashboard](https://app.usesuperflow.com) as well. Removing the config stops Cursor from calling the server, but it does not revoke the token Cursor already holds.
